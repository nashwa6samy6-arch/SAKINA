...
function openServicePanel(id) {
  ...
  document.getElementById('svc-panel').classList.add('active');
}

function closeServicePanel() {
  document.getElementById('svc-panel')?.classList.remove('active');
}
...
async function renderHosts() {
  ...
  try {
    ...
    tbody.innerHTML = _allHosts.map(h => `
      <tr>
        <td>${h.full_name || '—'}</td>
        <td>${h.email || '—'}</td>
...
async function openHostPanel(hostId) {
  ...
  document.getElementById('host-panel-status').innerHTML = '<span class="badge badge-approved">Active</span>';
...
function confirmSuspendHost(hostId, name) {
  openModal(
    ...
    'btn-panel-reject',
    async () => {
      await db.from('property_listings').update({ status: 'hidden' }).eq('landlord_id', hostId);
      closeHostPanel();
      renderHosts();
    }
  );
...
async function deleteService(id, name) {
  openModal(
    ...
    'btn-panel-reject',
    async () => {
      await db.from('service').delete().eq('service_id', id);
      renderServices();
    }
  );
}
